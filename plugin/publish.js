...
function begin(element, is_interface) {
    ...
    if ((element.scope === "global" || element.scope === "static" || element.isEnum && element.scope === undefined) && !options.module)
    {
        if(isClass(element) || isInterface(element) || isNamespace(element) || element.isEnum)
            write("export ");
    }
}
...
function writeFunctionSignature(element, isConstructor, isTypeDef) {
    ...
    paramNames.forEach(function(name, i) {
        ...
        //新增截取类
        var clsIdx = type.indexOf(".");
        var clsName = type.substring(clsIdx+1, type.length);
        write(name, !param.variable && param.optional ? "?: " : ": ", clsName);
...
function handleClass(element, parent) {
    ...
    if (!is_interface && !element.virtual)
    {
        ...
        writeln("{");
        ++indent;
        writeln("Message.setProperties(this,properties);");
        --indent;
        writeln("}");
    }
